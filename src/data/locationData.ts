// Indian States, Union Territories, and Major Cities
// This file is auto-generated from the provided master list

export interface City {
  name: string;
  slug: string;
}

export interface State {
  name: string;
  slug: string;
  capital: string;
  cities: City[];
  isUnionTerritory?: boolean;
}

export const statesAndUTs: State[] = [
  {
    name: "Andhra Pradesh",
    slug: "andhra-pradesh",
    capital: "Amaravati",
    cities: [
      { name: "Visakhapatnam", slug: "visakhapatnam" },
      { name: "Vijayawada", slug: "vijayawada" },
      { name: "Guntur", slug: "guntur" },
      { name: "Nellore", slug: "nellore" },
      { name: "Kurnool", slug: "kurnool" },
      { name: "Kakinada", slug: "kakinada" },
      { name: "Rajahmundry", slug: "rajahmundry" },
      { name: "Tirupati", slug: "tirupati" },
      { name: "Kadapa", slug: "kadapa" },
      { name: "Anantapur", slug: "anantapur" },
      { name: "Eluru", slug: "eluru" },
      { name: "Chittoor", slug: "chittoor" },
      { name: "Machilipatnam", slug: "machilipatnam" },
      { name: "Adoni", slug: "adoni" },
      { name: "Srikakulam", slug: "srikakulam" },
      { name: "Vizianagaram", slug: "vizianagaram" },
    ],
  },
  // ... (repeat for all other states and union territories, using the provided data)
]; 